...
function LabelBadge({
  ...
  const labelerUrl = `https://bsky.app/profile/${label.src}`;
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-1 rounded-full ${colorClasses} hover:opacity-80 transition-opacity cursor-pointer ${
        compact ? 'text-[10px] px-1.5 py-0' : 'text-xs px-2 py-0.5'
      }`}
      title={`Labeled by ${labelerName} - Click to view labeler`}
    >
      {label.severity === 'alert' && !gray && (
