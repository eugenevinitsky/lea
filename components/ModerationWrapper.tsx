...
export default function ModerationWrapper({
  ...
  // Render with alerts/informs (no blur, but show badges)
  return (
    <div>
      {/* Alert badges */}
      ...
      {/* Inform badges */}
      {contentModeration.informs.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-1">
          {contentModeration.informs.map((inform, i) => (
            <span
              key={i}
            ...
            >
              ...
              {inform.displayName || inform.label || inform.type}
            </span>
          ))}
        </div>
      )}
